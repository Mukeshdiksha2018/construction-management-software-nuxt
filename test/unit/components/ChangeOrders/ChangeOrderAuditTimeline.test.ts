...
describe('ChangeOrderAuditTimeline', () => {
  let wrapper: any
  let pinia: any
  ...
  const mockAuditLog = [
    ...
    },
    {
      ...
      user_image_url: 'https://example.com/avatar1.jpg',
      action: 'marked_ready',
      description: 'Change order marked as ready'
    },
    {
  ...
  const createWrapper = (props = {}) => {
    return mount(ChangeOrderAuditTimeline, {
      props: {
  ...
  describe('Action Titles', () => {
    it('should return correct action titles', () => {
      ...
      expect(wrapper.vm.getActionTitle('rejected')).toBe('Rejected')
    })
  ...
  describe('Action Icons', () => {
    it('should return correct icons for actions', () => {
      ...
      expect(wrapper.vm.getActionIcon('approved')).toContain('check-circle')
      expect(wrapper.vm.getActionIcon('rejected')).toContain('x-circle')
    })
  ...
  describe('Action Descriptions', () => {
    ...
    it('should generate description for created action', () => {
      ...
      expect(wrapper.vm.getActionDescription(log)).toBe('Change order was created')
    })

    it('should generate description for updated action', () => {
      wrapper = createWrapper()
      const log = { action: 'updated' }
      expect(wrapper.vm.getActionDescription(log)).toBe('Change order was updated')
    })

    it('should generate description for marked_ready action', () => {
      wrapper = createWrapper()
      const log = { action: 'marked_ready' }
      expect(wrapper.vm.getActionDescription(log)).toBe('Change order was marked as ready')
    })

    it('should generate description for approved action', () => {
      wrapper = createWrapper()
      const log = { action: 'approved' }
      expect(wrapper.vm.getActionDescription(log)).toBe('Change order was approved')
    })

    it('should generate description for rejected action', () => {
      wrapper = createWrapper()
      const log = { action: 'rejected' }
      expect(wrapper.vm.getActionDescription(log)).toBe('Change order was rejected')
    })

    it('should return default description for unknown actions', () => {
      wrapper = createWrapper()
      const log = { action: 'unknown' }
      expect(wrapper.vm.getActionDescription(log)).toBe('Change order was modified')
    })
  })
