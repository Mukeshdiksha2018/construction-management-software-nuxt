...
const VendorSelectStub = {
  name: "VendorSelect",
  props: ["modelValue", "corporationUuid"],
  emits: ["update:modelValue"],
  template:
    '<select class="vendor-select-stub" :value="modelValue" @change="$emit(\'update:modelValue\', $event.target.value)" />',
};
  ...
  return mount(ReceiptNoteForm, {
    ...
    global: {
      ...
      stubs: {
        ...
        ReceiptNoteItemsSelectionModal: { template: '<div class="items-selection-modal-stub"></div>' },
      },
    },
  });
};

describe("ReceiptNoteForm - Item Name Lookup from Preferred Items", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    // Default mock for receipt note items
  ...
  describe("Item Name Lookup Priority", () => {
    it("should prioritize item_name from preferred items by item_uuid", async () => {
      ...
      // Check that receipt items have correct item_name from preferred items
    ...
    it("should handle case-insensitive item_uuid matching in preferred items", async () => {
      const itemUuid1 = "ITEM-UUID-1"; // Uppercase
      
      // Mock preferred items with lowercase item_uuid
      const preferredItems = [
        {
      ...
      // Mock PO item with uppercase item_uuid
      const poItems = [
        {
          uuid: "po-item-1",
          item_uuid: itemUuid1, // Uppercase
      ...
      const item1 = items.find((item: any) => item.item_uuid === itemUuid1);
      expect(item1).toBeDefined();
      expect(item1?.item_name).toBe("Paper"); // Should match case-insensitively
    });
