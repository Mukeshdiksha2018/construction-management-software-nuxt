...
/**
 * Unit tests for the estimate items selection merging logic
 * These tests verify the core logic without requiring full component mounting
 */

describe("Estimate Items Selection - Merging Logic", () => {
  ...
  describe("Initial Import - Replace All Items", () => {
    ...
    it("should replace existing items when confirming initial import", () => {
      const existingItems = [
        { item_uuid: "existing-1", item_label: "Existing Item" },
      ];
      const selectedItems = [
        { item_uuid: "item-1", item_label: "Concrete" },
        { item_uuid: "item-2", item_label: "Rebar" },
  ...
  describe("Editing Existing Selection - Merge Items", () => {
    it("should preserve existing items and add new selected items when editing", () => {
      ...
      const selectedItems = [
        { item_uuid: "item-1", item_label: "Concrete" }, // Keep existing
        { item_uuid: "item-2", item_label: "Rebar" }, // Keep existing
        { item_uuid: "item-3", item_label: "Lumber" }, // Add new
      ];
      ...
      expect(item3?.item_label).toBe("Lumber");
    });
    ...
    it("should remove items that are deselected when editing", () => {
      const existingItems = [
        { item_uuid: "item-1", item_label: "Concrete" },
        { item_uuid: "item-2", item_label: "Rebar" },
        { item_uuid: "item-3", item_label: "Lumber" },
      ];

      const selectedItems = [
        { item_uuid: "item-1", item_label: "Concrete" }, // Keep
        { item_uuid: "item-3", item_label: "Lumber" }, // Keep
    ...
    it("should add new items without removing existing ones when editing", () => {
      ...
      const selectedItems = [
        { item_uuid: "item-1", item_label: "Concrete" }, // Keep existing
        { item_uuid: "item-2", item_label: "Rebar" }, // Keep existing
        { item_uuid: "item-4", item_label: "Steel" }, // Add new
      ];
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle duplicate items in selectedItems", () => {
      ...
      const selectedItems = [
        { item_uuid: "item-1", item_label: "Concrete" },
        { item_uuid: "item-1", item_label: "Concrete" }, // Duplicate
        { item_uuid: "item-2", item_label: "Rebar" },
      ];
  ...
  describe("Cancel Behavior Logic", () => {
    it("should distinguish between initial import and editing selection", () => {
      // Initial import: isEditingSelection = false
      const isEditingSelectionInitial = false;
      
      // Editing: isEditingSelection = true
      const isEditingSelectionEditing = true;

      expect(isEditingSelectionInitial).toBe(false);
      expect(isEditingSelectionEditing).toBe(true);
    });

    it("should preserve data when cancelling edit selection", () => {
      const existingItems = [
        { item_uuid: "item-1", item_label: "Concrete", description: "User edited" },
      ];
