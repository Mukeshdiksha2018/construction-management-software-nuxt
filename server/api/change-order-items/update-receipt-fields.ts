...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { data: receiptNote, error: receiptNoteError } = await supabaseServer
      ...
      .select("uuid, corporation_uuid, project_uuid, change_order_uuid, purchase_order_uuid")
    ...
    // Use change_order_uuid from receipt note (support legacy data in purchase_order_uuid)
    const coUuid = change_order_uuid || receiptNote.change_order_uuid || receiptNote.purchase_order_uuid;
    ...
    for (const item of items) {
      ...
      const receivedQty =
        item.received_quantity !== undefined && item.received_quantity !== null
          ...
          : null;
      const receivedQtyValue = Number.isFinite(receivedQty)
        ? receivedQty
        : null;
      ...
      if (receivedQtyValue !== null && item.unit_price !== undefined) {
        ...
        if (Number.isFinite(unitPrice)) {
          receivedTotal =
            Math.round((unitPrice * receivedQtyValue + Number.EPSILON) * 100) /
            100;
        }
      }

      const grnTotal =
        item.grn_total !== undefined && item.grn_total !== null
          ? Number.isFinite(parseFloat(String(item.grn_total)))
            ? parseFloat(String(item.grn_total))
            : null
          : null;

      const grnTotalWithCharges =
        item.grn_total_with_charges_taxes !== undefined &&
        item.grn_total_with_charges_taxes !== null
          ? Number.isFinite(
              parseFloat(String(item.grn_total_with_charges_taxes))
            )
            ? parseFloat(String(item.grn_total_with_charges_taxes))
            : null
          : null;
    ...
    const { data: existingRnis, error: fetchExistingError } =
      await supabaseServer
        .from("receipt_note_items")
        .select("uuid, item_uuid")
        .eq("receipt_note_uuid", receipt_note_uuid)
        .eq("item_type", "change_order")
        .in("item_uuid", itemUuidsToUpdate);

    if (fetchExistingError) {
      console.error(
        `[UpdateReceiptFields] Failed to fetch existing receipt_note_items:`,
        fetchExistingError
      );
      throw createError({
        statusCode: 500,
    ...
    // OPTIMIZATION: Batch update existing items using Promise.all for parallel updates
    if (itemsToUpdate.length > 0) {
      ...
      if (updateErrors.length > 0) {
        console.error(
          `[UpdateReceiptFields] Failed to update ${updateErrors.length} receipt_note_items:`,
          updateErrors
        );
        const firstError = updateErrors[0];
        throw createError({
          statusCode: 500,
          statusMessage: `Failed to update receipt note items: ${
    ...
    if (itemsToInsert.length > 0) {
      ...
      if (insertError) {
        console.error(
          `[UpdateReceiptFields] Failed to batch insert receipt_note_items:`,
          insertError
        );
        throw createError({
          statusCode: 500,
    ...
    if (fetchItemsError) {
      console.error(
        `[UpdateReceiptFields] Failed to fetch items for receipt_note_uuids update:`,
        fetchItemsError
      );
      ...
      (currentItems || []).forEach((currentItem: any) => {
        ...
        const currentArray = Array.isArray(currentItem.receipt_note_uuids)
          ...
          : typeof currentItem.receipt_note_uuids === "string"
          ? JSON.parse(currentItem.receipt_note_uuids)
          : [];
      ...
      if (updatePromises.length > 0) {
        ...
        if (updateErrors.length > 0) {
          console.error(
            `[UpdateReceiptFields] Failed to update receipt_note_uuids for ${updateErrors.length} items:`,
            updateErrors
          );
          // Don't throw - this is not critical
        }
    ...
    console.error("[change-order-items/update-receipt-fields] API error:", error);
